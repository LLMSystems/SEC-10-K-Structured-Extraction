...
- `prose`：尾段可直接做文字比對，納入 tail 直接驗證分母。
- `table / figure`：尾段常是表格 HTML、數字欄位或圖軸標記，不適合逐字比對；這類尾段改由下一個 item 的 `head` 覆蓋同一條邊界。

本資料集中，尾段分桶分布如下：

- `純文字尾段（prose） = 50/62`
- `表格尾段、由下一個 item 開頭覆蓋 = 10/62`
- `表格尾段、且為最後一個 item = 1/62`
- `圖像尾段、由下一個 item 開頭覆蓋 = 1/62`

因此：

- tail 直接驗證的總分母是 `50`，不是 `62`
- 非 `prose` 尾段共有 `12/62 (19.4%)`
- 但其中 `11/12` 屬於 `_covered`，邊界已由下一個 item 的 `head` 守住
- 真正完全未覆蓋的只有 `1/62 (1.6%)`
