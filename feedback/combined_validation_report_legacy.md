# Parser Validation Full Report

> 本文件整合兩份驗證報告，依閱讀順序先呈現 **External Reference Validation (Tier 2)**，再呈現 **Deterministic Validation (Tier 1)**。
> 原始來源：[external_reference_validation_report.md](./external_reference_validation/external_reference_validation_report.md) 、[deterministic_validation_report.md](./deterministic_validation/deterministic_validation_report.md)

## Reading Guide

1. **Part I — External Reference Validation (Tier 2)**
   說明如何用 VLM 與外部參照，驗證 parser 抽出的 item 起訖邊界是否正確，並附模型 benchmark、精確度、偵測率與成本。
2. **Part II — Deterministic Validation (Tier 1)**
   說明哪些條件是任何正確解析必然滿足的不變量，並用 34 份 Ground Truth 驗證其零誤殺與對應偵測率。

---

# Part I — External Reference Validation (Tier 2)

> 規則來源：[plan.md](./plan.md)（Tier 2 規則 7）　計畫：[validation_plan.md](./external_reference_validation/validation_plan.md)　與 [deterministic validation report](#part-ii--deterministic-validation-tier-1) 平行
> 程式與結果：[external_reference_validation/](./external_reference_validation/)（驗證器、資料集、報告）
...
本報告交付的是**已建置並完成驗證的來源 C**；A、B 仍列於計畫中（[validation_plan.md](./external_reference_validation/validation_plan.md) §4–5），作為後續工作。
...
**流程**（[vlm_first_last.py](./external_reference_validation/vlm_first_last.py)）：
...
並額外標註 `pdf + 每頁 PNG + item 起訖頁 + 內文 GT`（[dataset/](./external_reference_validation/dataset/)）。
這 5 份是 Tier 2 的驗證樣本。

---

## VLM 選型（[bench.md](./external_reference_validation/bench.md)）

驗證器本身包含 VLM 元件，因此模型選型會直接決定整體可靠度。選型流程如下：

1. **候選來源**：[bench.md](./external_reference_validation/bench.md)（根據 [llm-stats多模態排行](https://llm-stats.com/benchmarks/category/multimodal)：Rating / 價格 / context / 速度 / TTFT），
   據此選出最新開源/可負擔且多模態的候選。
...
詳見 [vlm_first_last_results.md](./external_reference_validation/vlm_first_last_results.md)。
...
程式見 [vlm_inject_eval.py](./external_reference_validation/vlm_inject_eval.py)，報告見 [report/detection/](./external_reference_validation/report/detection/)。
...
| 項目 | 路徑 | 說明 |
|---|---|---|
| 驗證資料集（5 份） | [dataset/](./external_reference_validation/dataset/) | pdf + 每頁 PNG + 起訖頁 + 內文 GT |
| 首尾段驗證器 | [vlm_first_last.py](./external_reference_validation/vlm_first_last.py) | 渲染對照、分桶、partial_ratio |
| VLM 呼叫封裝 | [vlm_reader.py](./external_reference_validation/vlm_reader.py) | OpenRouter + 快取 + reasoning 控制 |
| 模型 survey 結果 | [vlm_first_last_results.md](./external_reference_validation/vlm_first_last_results.md) | 14 模型 × 5 份 head/tail |
| 偵測力（錯誤注入） | [vlm_inject_eval.py](./external_reference_validation/vlm_inject_eval.py) ／ [report/detection/](./external_reference_validation/report/detection/) | §7.3 注入評估 |
| VLM 選型參考 | [bench.md](./external_reference_validation/bench.md) | llm-stats 多模態排行 |
...
# Part II — Deterministic Validation (Tier 1)

> 實作與數據：[deterministic_validation/](./deterministic_validation/)

## 結論

Tier 1 的目的，是驗證一組「**任何正確解析都必然滿足**」的結構性條件，並檢查這些條件是否足以作為高信心的 parser 驗證器。
...
因此，Tier 1 可以被視為一層高精確度的結構驗證：只要違反規則，即可直接判定解析出錯；而在規則適用範圍內，
它對主要 parser 失敗模式也具備完整的攔截能力。

## 定位

Tier 1 是一組**確定性不變量**：每條規則都描述了「任何正確解析必然滿足」的性質，
因此**只要違反，即可直接判定解析出錯**。它與統計式門檻的根本差異在於：
統計門檻回答的是「這份結果與歷史分布不同」（相關性），而 Tier 1 回答的是
...
Tier 1 鎖定的是**結構與幾何層級**的錯誤，並在此範圍內達到零誤報，且對主要失敗模式提供完整偵測。
更細緻的語意層查核，例如個別邊界字句是否精準、非核心小 Item 的取捨，則依設計交由 Tier 2 的外部參照機制處理；
...
| 錯誤資料（對抗集） | [`feedback/deterministic_validation/mutants/`](./deterministic_validation/mutants/) | 3,760 份注入錯誤的解析；索引見 `manifest.json` |
| 核心 Item 清單依據 | [`deterministic_validation/rules.py`](./deterministic_validation/rules.py) | 規則 3b 的 `CORE_ITEMS` 定義位置 |

**程式來源**

| 模組 | 檔案 | 職責 |
|---|---|---|
| 資料模型與 GT 載入 | [`deterministic_validation/model.py`](./deterministic_validation/model.py) | gold parse 結構、SEC 標準編號順序 |
| 四條規則 | [`deterministic_validation/rules.py`](./deterministic_validation/rules.py) | 規則定義與門檻 |
| 錯誤注入 | [`deterministic_validation/mutations.py`](./deterministic_validation/mutations.py) | 各 operator |
| 驗證主程式 | [`deterministic_validation/runner.py`](./deterministic_validation/runner.py) | 精確度、偵測率、關鍵診斷 |
| 說明文件 | [`deterministic_validation/README.md`](./deterministic_validation/README.md) | 結構與結果彙整 |
